...
<?php 
	$arDecorate = Array(
		"NAME" => '<a href="/scriptacid/admin/catalog/catalog.php?TYPE=#ID#">#NAME#</a>',
	);
?>
	<table>
		<thead>
			...
			<?php foreach ($arParams["SHOW_FIELDS"] as $field):?>
				<th>
					<?php echo LANG('CATALOG_TYPE_'.$field)?>
				</th>
			<?php endforeach?>
				<th>&nbsp;</th>
			</tr>
		...
		<?php foreach ($arResult as $n => $arItem):?>
			<tr <?php echo $n%2 ? 'class="altrow"' : ''?>>
				...
				<?php endforeach;?>
				<td><a href="?ACTION=EDIT&ID=<?php echo $arItem["ID"]?>">Изменить</a> <a href="?ACTION=DELETE&ID=<?php echo $arItem["ID"]?>">Удалить</a></td>
			</tr>
		<?php endforeach?>
	</table>
<?php else:?>
